...
import { enqueuePending } from "@/lib/signup-backup";
import {
  EXPERIENCIA,
  SISTEMA,
  TEMATICA,
...
export function RpgSignupForm({ onDone, quizTags = [], source }: Props) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // El servidor respondió pero con error (ej: falta configurar backend,
      // rate limit). Igual la guardamos local para no perderla.
      enqueuePending(payload);
      setStatus("queued");
      onDone?.();
    } catch {
      // Sin conexión o el servidor no respondió. Mismo tratamiento: se
      // guarda local y se reintenta sola cuando vuelva la red.
      enqueuePending(payload);
      setStatus("queued");
      onDone?.();
    }
